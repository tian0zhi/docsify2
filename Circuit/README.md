...
这是二阶线性非齐次方程，它的解答由特解和对应的齐次方程的通解组成。如果$i_s$为直流激励或正弦激励，则取稳态解$i^{'}$为特解，而通解$i_{''}$与零输入响应形式相同，再根据初始条件确定积分常数，从而得到全解。
